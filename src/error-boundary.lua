...
type callbackFn<T> = (element: T, index: number, array: {T}) -> boolean
type callbackFnWithThisArg<T, U> = (thisArg: U, element: T, index: number, array: {T}) -> boolean

local function ArraySome<T, U>(t: {T}, callback: callbackFn<T> | callbackFnWithThisArg<T, U>, thisArg: U?): boolean
...
local function ObjectIs(value1: any, value2: any): boolean
...
local function hasArrayChanged(a: {any}?, b: {any}?)
	...
	if a and b then
		return #a ~= #b or ArraySome(a, function(item, index)
			return not ObjectIs(item, b[index])
		end)
	end
